...
export function TaskSettingsPage({ onBack, onOpenTaskCenter }: Props) {
  ...
  const refresh = async () => {
    try {
      setLoading(true);
      const [data, remoteSettings] = await Promise.all([listPapers(), getTaskSettings().catch(() => null)]);
      setPapers(data);
      if (remoteSettings) {
  ...
  return (
    <div className="task-settings-page animate-fadeIn">
      ...
      {/* 试卷列表安排 */}
      <div className="card animate-fadeInUp stagger-2" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', padding: 'var(--space-5)' }}>
